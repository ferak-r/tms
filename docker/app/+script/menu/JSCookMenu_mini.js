...
returnStr = ' onmouseover="try {cmItemMouseOver (this,' + prefix + ',' + isMain + ',' + idSub + ',' + index + ')} catch(e){}" onmousedown="cmItemMouseDownOpenSub (this,' + index + ',' + prefix + ',' + orient + ',' + idSub + ')"'; else
returnStr = ' onmouseover="try {cmItemMouseOverOpenSub (this,' + prefix + ',' + isMain + ',' + idSub + ',' + orient + ',' + index + ')} catch(e){}" onmousedown="cmItemMouseDown (this,' + index + ')"'; return returnStr + ' onmouseout="try {cmItemMouseOut (this,' + nodeProperties.delay + ')} catch(e){}" onmouseup="cmItemMouseUp (this,' + index + ')"';}
function cmNoClickItem (item, prefix, isMain, idSub, orient, nodeProperties)
{ _cmItemList[_cmItemList.length] = item; var index = _cmItemList.length - 1; idSub = (!idSub) ? 'null' : ('\'' + idSub + '\''); orient = '\'' + orient + '\''; prefix = '\'' + prefix + '\''; return ' onmouseover="try {cmItemMouseOver (this,' + prefix + ',' + isMain + ',' + idSub + ',' + index + ')} catch(e){}" onmouseout="try{cmItemMouseOut (this,' + nodeProperties.delay + ')} catch(e){}"';}
...
function cmGetHorizontalAlign (obj, mode, p, subMenuWidth)
{
	...
	if (window.innerWidth) { 
		...
		return horiz;
	if (mode.charAt (0) == 'h') {
		//horiz = 'l';  // l = default direction for farsi menu --by ser
