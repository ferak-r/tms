...
switch($step){
	case 'transport':
		...
		//### paging
		$pagingsql = "SELECT COUNT(DISTINCT xtransportid)
					  FROM xxtransport
					  NATURAL LEFT JOIN xxcustoms
					  NATURAL LEFT JOIN xxtransportdocument
					  NATURAL LEFT JOIN (
...
//@tid=xtransportid,
		...
		$sql = "SELECT xtransportid, GROUP_CONCAT( CONCAT( CAST( xcounter AS CHAR ), '- ', xload ) SEPARATOR '<br>' ) AS xload, xtransportcode, xlaststatus, xdocumentcomment, xdocumentnumber,
				...
				NATURAL LEFT JOIN xxcustoms
			    NATURAL LEFT JOIN (
					SELECT xtransportid, GROUP_CONCAT(xdocumentnumber) AS xdocumentnumber
		...
		$col[] = array('<div style="font-size: 10px">
							ATA <small>(Arrival Port)</small>: {xetaarrivalport} <br>
							ARV Notice: {xataarrivalport}<br>
							ATA <small>(Exit Port)</small>: {xetaexitport} <br>
